...
- Model structure changes: edit `src/cliffordnet/tasks/imagenet1k.py` around `CliffordInteraction`, `CliffordBlock`, `CliffordNet`, and the `cliffordnet_*` builders.
...
3. Apply stable recipe edits in `configs/profiles/stable_imagenet1k.yaml`.
4. Move model-structure edits into `src/cliffordnet/tasks/imagenet1k.py` once the recipe-level signal is clear.
5. Use `configs/profiles/stress.yaml` for long HPC workload runs after the small runs are stable.
