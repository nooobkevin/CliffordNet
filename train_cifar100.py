"""
CliffordNet Training Script for CIFAR-100 with PyTorch Lightning + FSDP
Uses torchvision for automatic download
...
def cliffordnet_small_cifar(num_classes=100):
    """Small model for CIFAR-100 (~2.6M params)"""
    return CliffordNetCIFAR(
        img_size=32,
        embed_dim=160,
        depth=12,
...
def cliffordnet_base_cifar(num_classes=100):
    """Base model for CIFAR-100 (~5M params)"""
    return CliffordNetCIFAR(
        img_size=32,
        embed_dim=192,
        depth=16,
...
class CliffordNetLightning(L.LightningModule):
    def __init__(
        ...
        warmup_epochs: int = 5,
        max_epochs: int = 200,
        label_smoothing: float = 0.0,  # Default to 0 to match single-GPU training
    ):
        super().__init__()
        ...
        # Build model
        model_builders = {
            "nano": cliffordnet_nano_cifar,
            "small": cliffordnet_small_cifar,
            "base": cliffordnet_base_cifar,
        }
...
# ============================================================================
# Data Module with auto-download
# ============================================================================
...
def main():
    ...
    base_batch_size = 128
    if args.lr is None:
        # Linear scaling: lr = base_lr * (effective_batch_size / base_batch_size)
        args.lr = args.base_lr * (effective_batch_size / base_batch_size)

    # Print configuration
    print("=" * 60)
    print("CliffordNet Training on CIFAR-100 with DDP")
    print("=" * 60)
    ...
    callbacks = [
        ModelCheckpoint(
            ...
            filename="cliffordnet-cifar100-{epoch:02d}-{val/acc:.4f}",
            monitor="val/acc",
            mode="max",
            save_top_k=3,
            save_last=True,
    ...
    logger = TensorBoardLogger(
        ...
        name="cliffordnet_cifar100",
    )

    # DDP Strategy for multi-GPU (supports gradient clipping)
