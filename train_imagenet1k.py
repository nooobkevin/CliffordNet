"""
CliffordNet Training Script for ImageNet-1k with PyTorch Lightning + FSDP
Uses HuggingFace datasets for automatic download
...
# ============================================================================
# Model Configurations for ImageNet-1k
# ============================================================================

def cliffordnet_small(num_classes=1000):
    """Small model for ImageNet-1k (~6M params)"""
    return CliffordNet(
        img_size=224,
        embed_dim=192,
        depth=12,
...
def cliffordnet_base(num_classes=1000):
    """Base model for ImageNet-1k (~15M params)"""
    return CliffordNet(
        img_size=224,
        embed_dim=384,
        depth=16,
...
def cliffordnet_large(num_classes=1000):
    """Large model for ImageNet-1k (~40M params)"""
    return CliffordNet(
        img_size=224,
        embed_dim=512,
        depth=24,
...
# ============================================================================
# Data Module with HuggingFace auto-download
# ============================================================================
...
class ImageNet1kDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        print(f"Downloading ImageNet-1k to {self.data_dir}...")
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            ...
            self.train_dataset = HFImageNetDataset(
                dataset["train"],
                transform=self.train_transform
            )
            self.val_dataset = HFImageNetDataset(
                dataset["validation"],
...
# ============================================================================
# DDP Configuration (simpler and more stable, supports gradient clipping)
# ============================================================================
...
def main():
    ...
    parser.add_argument("--base-lr", type=float, default=1e-3,
                        help="Base learning rate for batch size 64")
    ...
    # Auto-scale learning rate based on effective batch size (Linear Scaling Rule)
    effective_batch_size = args.batch_size * args.num_gpus
    base_batch_size = 64
    if args.lr is None:
        args.lr = args.base_lr * (effective_batch_size / base_batch_size)

    # Print configuration
    print("=" * 60)
    print("CliffordNet Training on ImageNet-1k with DDP")
    print("=" * 60)
    ...
    data_module = ImageNet1kDataModule(
    ...
    logger = TensorBoardLogger(
        ...
        name="cliffordnet_imagenet1k",
    )

    # DDP Strategy (supports gradient clipping)
    strategy = get_strategy(args.num_gpus)
